...
import numpy as np
import h5py
from baseline_final_model import build_res_unet, get_mask
...
dist_from_inlet = np.full(top_dry.shape, np.squeeze(np.load('in/dist_from_inlet.npy'))[:,::-1])
...
dist_from_inlet_rev = np.full(top_dry_rev.shape, np.squeeze(np.load('in/dist_from_inlet.npy')))
...
model = build_res_unet((128, 256, 4), (128, 256, 2), filters=32)
