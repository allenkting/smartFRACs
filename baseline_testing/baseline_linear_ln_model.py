...
def res_block(x, nb_filters, strides):
    ...
    res_path = Conv2D(filters=nb_filters[0], kernel_size=(3, 3), strides=strides[0])(res_path)
    res_path = LayerNormalization()(res_path)
    res_path = Activation(activation='relu')(res_path)
    res_path = pad_layer(res_path)
    res_path = Conv2D(filters=nb_filters[1], kernel_size=(3, 3), strides=strides[1])(res_path)
...
def encoder(x, filters):
    ...
    main_path = Conv2D(filters=filters, kernel_size=(3, 3), strides=(1, 1))(main_path)
...
def build_res_unet(input_shape, 
    ...
    path = Conv2D(filters=output_shape[2], kernel_size=(1, 1))(path)
